// MyTaxi.js - MyTaxi Backend (Customer, Driver, Admin + JWT + Hashing + Dynamic Fare)
...
app.post("/rides", authenticate, authorize(["customer"]), async (req, res) => {
    ...
    let driverObj;
    try {
        driverObj = await db.collection("drivers").findOne({ _id: new ObjectId(driver) });
        if (!driverObj) {
            return res.status(404).json({ error: "Driver not found" });
        }
        ...
        return res.status(500).json({ error: "Error checking driver availability" });
    }
    ...
    try {
        ...
        // 🔄 Optionally set driver to "unavailable" immediately
        await db.collection("drivers").updateOne(
            { _id: new ObjectId(driver) },
...
app.patch("/rides/:id", authenticate, authorize(["driver"]), async (req, res) => {
    ...
    // Update driver earnings and set status to available
    await db.collection("drivers").updateOne(
...
app.get("/", (req, res) => {
    res.send("\u2705 MyTaxi System API is running.");
});


// GET all registered drivers (no auth for now)
app.get("/drivers", async (req, res) => {
    try {
        const drivers = await db.collection("drivers").find({}, { projection: { name: 1, licensePlate: 1 } }).toArray();
